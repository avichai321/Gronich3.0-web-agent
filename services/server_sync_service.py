        ...
        try:
            if job["job_type"] == "file_copy_browse":
                ...
                if not session.get("success"):
                    self.submit_job_result(job["job_id"], "failed", result={}, message=session.get("message", "Create session failed"))
                    return
            ...
            self.submit_job_result(job["job_id"], "failed", result={}, message=f"Unsupported job type: {job['job_type']}")
